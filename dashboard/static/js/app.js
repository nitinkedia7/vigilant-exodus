...
function initMap() {
  ...
  var origin = {lat:37.782551, lng: -122.445368};
...
ClickEventHandler.prototype.calculateAndDisplayRoute = function (latLng) {
  ...
  this.directionsService.route({
    origin: this.origin,
    // destination: "Conservatory of Flowers 100 John F Kennedy Dr San Francisco, CA 94118",
    // origin: 'California Pacific Medical Center Davies Campus',
    destination: latLng,
    // destination: {
    //   // placeId: placeId
    //   latLng: latLng
...
ClickEventHandler.prototype.getPlaceInformation = function () {
  ...
  //       <ul>
  //         <li>Bedrooms: ${feature.getProperty('water')}</li>
  //         <li>Bathrooms: ${feature.getProperty('food')}</li>
  //         <li>Car spaces: ${feature.getProperty('medicine')}</li>
  //     `;
  // content += '</ul></div>';
  me.infowindow.close();
  // me.infowindow.setContent(content);
  me.infowindow.setPosition(position);
