...
function initMap() {
    ...
    timeoutHandler = setTimeout(() => {
      ...
      map.data.loadGeoJson(hazard_url, null, (features) => {
        ...
        features.map((feature) => {
          hazards.push({
            location: feature.getGeometry().get(0),
  ...
  var centerControl = new CenterControl(centerControlDiv, map, "Check-In");

  centerControlDiv.index = 1;
  map.controls[google.maps.ControlPosition.LEFT_TOP].push(centerControlDiv);

  var MissingPersonDiv = document.createElement('div');
  var MissingPersonControl = new CenterControl(centerControlDiv, map, "Find Missing Person");

  MissingPersonDiv.index = 1;
  map.controls[google.maps.ControlPosition.LEFT_TOP].push(MissingPersonDiv);
}
...
function CenterControl(controlDiv, map, text) {
  ...
  if (text == "Check-In") controlUI.style.marginTop = '30px';
  ...
  controlUI.addEventListener('click', function () {
    ...
    else if (text == "Find Missing Person")
      window.location.href = "/missingperson/";
  });
}


// Heatmap data: 500 Points
function getPoints(hazards) {
  randomHazards = [
